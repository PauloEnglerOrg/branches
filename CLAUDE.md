...
- Node shape:
  `{ id, parent (null = free-floating), x, y, title, size (-1 XXS, 0 XS, 1 S, 2 M default, 3 L, 4 XL; read with `sizeOf()` since 0 is falsy), color (optional hex; a coloured box is filled, text colour from `onColor`), tint (optional -80..80: brightness; `shown(n)` = `shade(color, tint)` mixes black/white, keeping the hue), done, due ("YYYY-MM-DD" or "YYYY-MM-DDTHH:MM"), items: [{id, text, done}], links: [{url, label}], side (optional 't'|'r'|'b'|'l': which side of the parent the connector leaves from), collapsed (optional true: hides all descendants), w/h (optional custom box width / min-height set with the corner handle), repeat (optional {every, unit day|week|month|year, days, dom, anchor, from}: ticking rolls `due` on via `nextDue()`), notify (optional {to:[emails], days:[days before], sent:{'<due>|<days>':iso}}: email reminders sent by the daily GitHub Action `scripts/send-reminders.mjs`), task (optional true: shows a tick-box; `isTask()` also treats untagged done boxes as tasks) }`
...
- Isolate: `focusStack` (device-only) limits `visible()` to one box and its descendants; `#focusbar` breadcrumb, Escape steps out, new boxes are created inside the isolated box
- Keeping the layout: `followersOf(s)` = s's own branches below it (follow its bottom edge) or to its right (follow its right edge) at any distance, plus unconnected boxes stacked under it (gap ≤ `STACK_GAP`) and boxes beside it in the same row (`rowRight`, follow its right edge, with their branches). Group resize also shifts the resized boxes against each other by row/column. `followPlan`/`applyFollow` are used by resize, align/distribute/match, text size and reset size; `render()` also compares sizes before/after and `autoFollow` pushes followers of boxes that grew or shrank. `renderQuiet()` skips that for undo/redo and remote changes
