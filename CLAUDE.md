...
- Stack following: `followPlan(srcs)` records boxes stacked under each source (gap ≤ `STACK_GAP`); `applyFollow` moves them by the source's x shift and bottom-edge shift. Used by resize, align/distribute/match, text-size and reset-size. Also `render()` compares heights before/after and `autoFollow` pushes stacks of boxes whose height changed (typing, checklist...); `renderQuiet()` skips that for undo/redo and remote changes
